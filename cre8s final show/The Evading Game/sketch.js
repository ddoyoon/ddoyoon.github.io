...
obstacleList.prototype.drawObstacle = function() {
  ...
  for(i = 0; i < this.length; i = i + 1){
    fill(this.color[i]);
    rect(this.x[i], this.y[i]+catH/2-this.height[i]/2, this.width[i], this.height[i], 10);
    fill(0);
    textSize(18);
    textAlign(CENTER, BASELINE);
    text(this.text[i], this.x[i], this.y[i]+catH/2-this.height[i]/2+5);
    //text(":)", this.x[i], this.y[i]+catH/2-this.height[i]/2);
...
obstacleList.prototype.die = function(xpos, ypos){
  ...
  for(i = 0; i < this.length; i = i + 1){
    ...
    if( Math.abs(this.x[i] - xpos) < catW/2 - 35 + this.width[i]/2 && Math.abs(this.y[i] - ypos) < this.height[i]/2 -23 + catH/2){

      if(!captured) {
        captured = true;
        //console.log(captured);   //실행안됨 힝
      } 
      else {
        noLoop();
      }
...
function setup() {
  ...
  catX = fullX/2 - w/4; catY = fullY/2+5;
  imageMode(CENTER);
...
function draw() {
  ...
  if(!play && captured) {
    ...
    stroke(0);
    line(fullX/2 -w/2, fullY/2 + catH/2, fullX/2 +w/2, fullY/2 + catH/2);
    noStroke();
  }
  else if(play) {
    rectMode(CORNER);
    ...
    fill(121,169,51);
    rect(fullX/2-w/2, fullY/2+catH/2, w, h/2- catH/2);
    fill(84,209,255);
    rect(fullX/2-w/2, fullY/2-h/2, w, h/2 + catH/2);
    imageMode(CENTER);
  ...
  if(toggle == -1){
    ...
    if(!obstacle.die(catX, catY)) {
      obstacle.update(5);
      
      for(var j = 0; j < obstacle.length; j++){
        if(obstacle.success[j] == false && obstacle.x[j] < catX){
